...
// ─────────────────────────────────────────────
//  Handle form actions (add room, toggle visibility)
// ─────────────────────────────────────────────
$msg = "";
...
if (isset($_GET['visible']) && $_GET['visible'] !== '') {
  $where[] = "r.is_visible = ?";
  $params[] = intval($_GET['visible']);
  $typestr .= 'i';
}

$where_sql = $where ? 'WHERE ' . implode(' AND ', $where) : '';
$sql = "SELECT r.*, t.name AS type 
        FROM rooms r 
...
<head>
  ...
  <style>
    ...
    h3, h5 { color: var(--brand-red); }

    .action-row { display:flex; flex-wrap:wrap; gap:6px; justify-content:center; }
    .text-break { word-break: break-word; }
    .card .form-control, .card .form-select { min-height: calc(1.5em + .75rem + 2px); }
  </style>
</head>
          ...
          <div class="card-body">
            ...
            <form method="post" class="row g-2">
              ...
              <div class="col-12 col-md-3">
                <input name="room_number" class="form-control" placeholder="Room Number">
              </div>
            ...
            <form method="get" class="row g-2">
              ...
              <div class="col-12 col-md-3">
                <select name="room_type" class="form-select">
                  ...
                  <?php $types->data_seek(0); while($t2=$types->fetch_assoc()): ?>
                    <option value="<?=$t2['id']?>" <?=(!empty($_GET['room_type']) && $_GET['room_type']==$t2['id'])? 'selected':''?>><?= htmlspecialchars($t2['name']) ?></option>
                  <?php endwhile; ?>
                </select>
              </div>
              ...
              <table class="table table-bordered table-hover align-middle">
                ...
                <tbody>
                  <?php while($r=$roomsRes->fetch_assoc()): ?>
                    <tr>
                      <td><?= intval($r['id']) ?></td>
                      <td class="text-break"><?= htmlspecialchars($r['room_number']) ?></td>
                      <td class="text-break"><?= htmlspecialchars($r['type']) ?></td>
